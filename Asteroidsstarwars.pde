...
PImage[] ImperialImg = new PImage[3];
...
void setup(){
 background(#313131);
 size(800,500);
 font = createFont("Cambria-BoldItalic-48", 32);
 ImperialImg[0] = loadImage("Caza.png");
 ImperialImg[1] = loadImage("Lanzadera.png"); 
 ImperialImg[2] = loadImage("Destructor.png"); 
 rocket = loadImage("Halcon.png");
 frameRate(24);
 lives = 3;
...
void reset(){
 ...
 for(int i = 0; i <numAsteroids + diffCurve*stage; i++){
  ...
  asteroids.add(new Asteroid(position, startingRadius, ImperialImg, stage));
 }
 darkCounter = 0;
}
...
class Bullet{
 ...
 int timeOut = 24 * 2;
 float alpha;
 PImage img = loadImage("Disparo.png");
...
class Ship{
 ...
 float drag = .9;
 float r = 15;
 PImage img = loadImage("Halcon.png");
